"""
  ...
  were performing a caesar cipher of the string "tim" with
  offset = 2 you would get "rgk". "t" is
  shifted two positions to "r", "i" is shifted two
  positions to "g" and "m" is shifted two positions to
  "k".
  In the situation where the shift of a character results in it being a position
  before "a" the positions wrap and the next character should be
  "z". For example, the caesar cipher of "ab" with
  offset = 2 would be "yz".
  offset will always be a positive integer that is no greater than
  26 and the input string will only contain lowercase letters.
...
encoded_string = caesar_cipher("abcdefghijklmnopqrstuvwxyz", 3)
